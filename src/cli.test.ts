...
describe("CLI", () => {
  ...
  describe("--help", () => {
    it("prints help and exits 0 with --help", () => {
      ...
      expect(stdout).toContain("trulocal");
      ...
      expect(stdout).toContain("TRUELOCAL_STATE_DIR");
    });
  ...
  describe("TRUELOCAL=0 bypass", () => {
    it("runs command directly when TRUELOCAL=0 is set", () => {
      const { status, stdout } = run(["myapp", "node", "-e", "console.log('hello')"], {
        env: { TRUELOCAL: "0" },
      });
      expect(status).toBe(0);
    ...
    it("runs command directly when TRUELOCAL=skip is set", () => {
      const { status, stdout } = run(["myapp", "node", "-e", "console.log('bypassed')"], {
        env: { TRUELOCAL: "skip" },
      });
      expect(status).toBe(0);
    ...
    it("does not bypass proxy commands when TRUELOCAL=0 is set", () => {
